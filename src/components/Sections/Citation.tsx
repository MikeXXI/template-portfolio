...
import {SectionId, citation} from '../../data/data';
import {Citation} from '../../data/dataDef';
import useInterval from '../../hooks/useInterval';
import useWindow from '../../hooks/useWindow';
import QuoteIcon from '../Icon/QuoteIcon';
import Section from '../Layout/Section';

const Citations: FC = memo(() => {
  ...
  const {imageSrc, citations} = citation;
  ...
  const next = useCallback(() => {
    if (activeIndex + 1 === citations.length) {
      setCitation(0)();
    } else {
      setCitation(activeIndex + 1)();
    }
  }, [activeIndex, setCitation, citations.length]);
  ...
  // If no citations, don't render the section
  if (!citations.length) {
    return null;
  }

  return (
    <Section noPadding sectionId={SectionId.Citations}>
      <div
          ...
          <div className="flex flex-col items-center gap-y-6 rounded-xl bg-gray-800/60 p-6 shadow-lg">
            <div
              ...
              {citations.map((citation, index) => {
                ...
                return (
                  <Citation isActive={isActive} key={`${citation.name}-${index}`} citation={citation} />
                );
              })}
            </div>
            <div className="flex gap-x-4">
              {[...Array(citations.length)].map((_, index) => {
                const isActive = index === activeIndex;
                return (
                  <button
                    className={classNames(
                    ...
                    onClick={setCitation(index)}></button>
                );
              })}
            </div>
          </div>
...
const Citation: FC<{citation: Citation; isActive: boolean}> = memo(
  ({citation: {text, name, image}, isActive}) => (
    <div
      className={classNames(
